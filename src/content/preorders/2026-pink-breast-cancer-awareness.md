---
title: Pink Jersey Drop
eyebrow: Breast Cancer Awareness
status: open
closesAt: 2026-09-28
price: 125
priceNote: shipped anywhere in the US
variants:
  - name: V1
    description: Cloud panel front, name and number on the back.
    imageAlt: The Leftovers V1 jersey in pink with cloud panelling
  - name: V2
    description: Full ruyi-cloud print with lantern detailing.
    imageAlt: The Leftovers V2 jersey in pink with full cloud print
# These strings must match the Google Form's options exactly.
# Note the form offers no youth sizes.
sizes: [Small, Medium, Large, X-Large, 2XL, 3XL]
deliveryOptions:
  - Ship to my address
  - Pick up at the team field
customisation:
  nameOnBack: true
  numberOnBack: true
cause:
  name: National Breast Cancer Foundation
  url: https://www.nbcf.org/
  donationPerUnit: 25
  goalUnits: 40
  unitsSold: 0
paymentMethods:
  - label: Venmo
  - label: Zelle
  - label: Cash in person
seo:
  title: Pink Jersey Pre-Order — Breast Cancer Awareness
  description: >-
    Pre-order The Leftovers pink V1 or V2 jersey. $25 per jersey goes to the
    National Breast Cancer Foundation, with a goal of $1,000 this October.
---

We're doing a pink colourway of the V1 and V2 jerseys for Breast Cancer
Awareness Month. **$25 from every jersey sold goes to the National Breast
Cancer Foundation**, and the goal is 40 jerseys — $1,000 donated this October.

Mockups on this page aren't final; the artwork will shift slightly before
print.

Orders close on the deadline above and everything prints in one batch after
that. There's no minimum to hit for the run to happen.
